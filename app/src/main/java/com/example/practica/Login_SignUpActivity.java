...
public class Login_SignUpActivity extends AppCompatActivity {
    ...
    private void loginUser() {
        ...
        if (validateInputs(login, password)) {
            new Thread(() -> {
                SQLiteDatabase db = dbHelper.getReadableDatabase();
                String[] columns = {DataBaseOfUsers.UserTable._ID, DataBaseOfUsers.UserTable.PASSWORD};
                    ...
                    if (cursor.moveToFirst()) {
                        ...
                        if (password.equals(storedPassword)) {
                            runOnUiThread(() -> {
                                ...
                                String username = cursor.getString(cursor.getColumnIndexOrThrow(DataBaseOfUsers.UserTable.USERNAME));
